...
switch ($task) {
	...
	case 'newdir':
		if (ini_get('safe_mode')=='On') {
			mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "В режиме SAFE MODE запрещено создание каталогов, что является проблемой." );
		} else {
...
function create_folder($dirPath) {
	...
	if(strlen($folder_name) >0) {
		if (eregi("[^0-9a-zA-Z_]", $folder_name)) {
			mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Название каталога может содержать только символы алфавита и не должно содержать пробелов." );
		}
...
function delete_folder($listdir) {
	...
	if ($entry_count < 1) {
		...
		echo '<font color="red">Невозможно удалить, т.к. не пустой!</font>';
	}
}
...
function do_upload($file, $dest_dir) {
	...
	if (empty($file['name'])) {
		mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Не выбран файл для загрузки" );
	}
