...
class ps_csv {
	...
	***************************************************************************/
	...
	function RetrieveConfigFields() {
		...
		if ($this->price_list_only) {
			...
			if ($this->unsupported_fields != "") {
				$vmLogger->err('Файл CSV содержит неправельные поля: '.$this->unsupported_fields);
				return false;
			}
			if ($this->debug) $this->csv_debug['message'] .= 'Сначала используйте строку для установки неоходимого поля<br />';
		}
		else {
			...
			if ($this->debug) $this->csv_debug['message'] .= 'Используйте БД для настройки<br />';
		}
		...
		if (array_key_exists("product_publish", $this->required_fields)) {
			...
			if ($this->debug) $this->csv_debug['message'] .= 'Используйте поле публикации продукта<br />';
		}
	}
	...
	function PriceListUploadOnly(&$d) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Режим - обновление прайс-листа <br />';
		// Check if the number of configured fields is the same as the number of fields in the CSV file
		
		if (2 > count($data)) {
			$d['csv_stats']['incorrect']['message'] .= "<strong>Неправильное количество колонок</strong>";
			$data = false;
			return false;
		}
		else {
			...
			foreach( $this->required_fields as $fieldname => $ordering ) {
				if (empty($data[$ordering-1])) {
					$d['csv_stats']['incorrect']['message'] .= "Строка $this->line: содержит пустое значение<br />";
			...
			if ($this->debug) $this->csv_debug['message'] .= '<hr>Обрабатываем артикул: '.$product_details->product_sku.'<br />';
			...
			if ($this->debug) $this->csv_debug['message'] .= 'Проверяем есть ли товар: '.htmlentities($q,ENT_QUOTES,vmGetChsrset()).'<br />';
			$dbp->query($q);
			...
			if ($dbp->next_record()) {
				...
				****************************/
				...
				$q .= "WHERE product_sku='" . $product_details->product_sku . "'";
				if ($this->debug) $this->csv_debug['message'] .= 'Обновлены товары с текущим временем: <a onclick="switchMenu(\''.$product_details->product_sku.'_mod_timestamp\');" title="Показать/Скрыть запрос">Показать/Скрыть запрос</a><div id="'.$product_details->product_sku.'_mod_timestamp" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q,ENT_QUOTES,vmGetCharset()).'</div><br />';
				if ($dbu->query($q)) {
					...
					if ($this->debug) $this->csv_debug['message'] .= 'Обновление цены товара: '.htmlentities($q,ENT_QUOTES,vmGetChsrset()).'<br />';
					if ($dbpp->query($q)) {
						// Add report for this line to message
						$d['csv_stats']['updated']['message'] .= "Строка $this->line: <strong>Обновлен</strong> Артикул товара : $product_details->product_sku<br />";
						$d['csv_stats']['updated']['count']++;
					}
					else  {
						$d['csv_stats']['incorrect']['message'] .= "Строка $this->line: Артикул товара: $product_details->product_sku невозможно обновить<br />";
						$d['csv_stats']['incorrect']['count']++;
					}
				}
				else {
				...
				$d['csv_stats']['incorrect']['message'] .= "Строка $this->line: Артикул товара: $product_details->product_sku не существует<br />";
				$d['csv_stats']['incorrect']['count']++;
			}
		}
		return true;
	...
	function NormalUpload(&$d) {
		...
		if (count($this->required_fields) != count($data)) {
			...
			if ($this->debug) $this->csv_debug['message'] .= '<hr>Обрабатываем артикул: '.$product_details->product_sku.'<br />';
			if ($this->debug) $this->csv_debug['message'] .= 'Режим стандартной загрузки<br />';
			...
			if (!empty($this_error)) {
				$d['csv_stats']['incorrect']['message'] .= "Строка $this->line: $this_error<br />";
				$d['csv_stats']['incorrect']['count']++;
				$this_error = "";
			}
			else if (!$product_details->product_sku) {
				$d['csv_stats']['incorrect']['message'] .= "Строка $this->line: Артикул товара не определен<br />";
				$d['csv_stats']['incorrect']['count']++;
			}
			else {
				$timestamp = time();
				...
				$q = "SELECT product_id FROM #__{vm}_product ";
				$q .= "WHERE product_sku='".$product_details->product_sku."'";
				if ($this->debug) $this->csv_debug['message'] .= 'Проверка - есть ли товар: <a onclick="switchMenu(\''.$product_details->product_sku.'_exist_product\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="'.$product_details->product_sku.'_exist_product" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q,ENT_QUOTES,vmGetCharset()).'</div><br />';
				$dbp->query($q);
				// Check if the SKU exists
				if ($dbp->next_record()) {
					/** Andrea
					...
					*************************************/
					if (!$this->overwrite_existing_data) {
						if ($this->debug) $this->csv_debug['message'] .= 'Пропущен продук, поскольку мы не записываем поверх данные: '.$data[$this->csv_fields["product_sku"]["ordering"]-1].'<br />';
						$d['csv_stats']['skipped']['message'] .= "Строка $this->line: <strong>Данные остались</strong> Продукт SKU: ".$data[$this->csv_fields["product_sku"]["ordering"]-1]."<br />";
						...
						****************************/
						if ($product_details->product_delete == "Y") {
							if ($this->debug) $this->csv_debug['message'] .= 'Продукт удален: '.$product_details->product_sku.'<br />';
							if ($product_details->product_id) {
								...
								if ($delete_product) {
									$d['csv_stats']['deleted']['message'] .= "Строка $this->line: <strong>Удален</strong> Продукт SKU: $product_details->product_sku<br />";
									$d['csv_stats']['deleted']['count']++;
								}
							}
							else {
								$d['csv_stats']['incorrect']['message'] .= "Строка $this->line: <strong>Не найден для удаления</strong> Продукт SKU: $product_details->product_sku<br />";
								$d['csv_stats']['incorrect']['count']++;
							}
						}
						else {
							...
							if (!$this->ProductQuery("update")) {
								$d['csv_stats']['incorrect']['message'] .= "Строка $this->line: <strong>Неправильный</strong> продукт SKU: $product_details->product_sku не мог быть обновлен<br />";
								$d['csv_stats']['incorrect']['count']++;
								if ($this->debug) $this->csv_debug['message'] .= 'Не получилось обновить цену на продукт<br />';
								continue;
							}
							...
							***********************************************************************************/
							...
							if (isset($this->required_fields["attributes"])) {
								if( !empty($data[$this->csv_fields["attributes"]["ordering"]-1])) {
									...
									if ($this->debug) $this->csv_debug['message'] .= 'Добавление атрибутов: <a onclick="switchMenu(\''.$product_details->product_sku.'_add_attributes\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="'.$product_details->product_sku.'_add_attributes" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
									$dbu->query($q);
									while(list(,$val) = each($attributes)) {
										...
										if ($this->debug) $this->csv_debug['message'] .= '<a onclick="switchMenu(\''.$product_details->product_sku.'_add_attributes.'.$i.'\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="'.$product_details->product_sku.'_add_attributes" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
										$dbu->query($q);
										$i++;
									}
								}
							...
							******************************************************************************************/
							...
							if ($product_details->attribute_values) {
								if ($this->debug) $this->csv_debug['message'] .= 'Добавление значений атрибутов<br />';
								...
								if ($this->debug) $this->csv_debug['message'] .= 'Удалено старое значение:<a onclick="switchMenu(\''.$product_details->product_sku.'_add_attribute_values\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="'.$product_details->product_sku.'_add_attribute_values" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
								$dbu->query($q);
								while(list(,$val) = each($attribute_values)) {
									...
									if ($this->debug) $this->csv_debug['message'] .= 'Выделить новое значение: <a onclick="switchMenu(\''.$product_details->product_sku.'_add_attribute_value'.$i.'\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="'.$product_details->product_sku.'_add_attribute_value'.$i.'" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
									$dbu->query( );
									$i++;
								}
							}
							...
							*************************************************/
							...
							if ($product_details->manufacturer_name || $product_details->manufacturer_id) {
								if ($this->debug) $this->csv_debug['message'] .= 'Проверка производителя<br />';
								$this->process_manufacturer($product_details->product_id);
							}
							
							// Add report for this line to message
							$d['csv_stats']['updated']['message'] .= "Строка $this->line: <strong>Обновлен</strong> продукт SKU: $product_details->product_sku<br />";
							$d['csv_stats']['updated']['count']++;
						}
					}
				}
				else if ($product_details->product_delete != "Y") {
					if ($this->debug) $this->csv_debug['message'] .= 'Добавлено новых товаров<br />';
					...
					*************************************************/
					if (!$this->ProductQuery("add")) {
						$d['csv_stats']['incorrect']['message'] .= "Строка $this->line: <strong>Неправильный</strong> артикул товара: $product_details->product_sku не получилось добавить<br />";
						$d['csv_stats']['incorrect']['count']++;
						continue;
					}
					...
					***********************************************************************************/
					...
					if ($product_details->attributes) {
						if ($this->debug) $this->csv_debug['message'] .= 'Добавление атрибута<br />';
						$attributes = explode( "|", $data[$this->csv_fields["attributes"]["ordering"]-1] );
						$i = 0;
						while(list(,$val) = each($attributes)) {
							$values = explode( "::", $val );
					...
					******************************************************************************************/
					// Check if the category_path is to be added
					if ($product_details->attribute_values) {
						if ($this->debug) $this->csv_debug['message'] .= 'Добавление значения атрибута<br />';
						$attribute_values = explode( "|", $data[$this->csv_fields["attribute_values"]["ordering"]-1] );
						$i = 0;
						while(list(,$val) = each($attribute_values)) {
							$values = explode( "::", $val );
					...
					// Add report for this line to message
					$d['csv_stats']['added']['message'] .= "Строка $this->line: Продукт SKU: ".$product_details->product_sku."<br />";
	...
	function MultiplePricesUpload(&$d) {
		...
		if ($product_details->price_delete) {
			...
			if ($this->debug) $this->csv_debug['message'] .= 'Получить ID цены на продукт: <a onclick="switchMenu(\'product_price_id_'.$this->line.$product_details->product_sku.$product_details->product_id.'\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_price_id_'.$this->line.$product_details->product_sku.$product_details->product_id.'" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
			$db->query($q);
			if (!$db->next_record()) {
				$d['csv_stats']['incorrect']['message'] .= "Строка $this->line: <strong>Цена не найдена</strong> продукт SKU: ".$product_details->product_sku."<br />";
				$d['csv_stats']['incorrect']['count']++;
			}
			else {
				$product_price_id = $db->f('product_price_id');
				if (!$ps_product_price->delete_record($product_price_id, $d)) {
					$d['csv_stats']['incorrect']['message'] .= "Строка $this->line: <strong>Цена не может быть удалена</strong> продукт SKU: ".$product_details->product_sku."<br />";
					$d['csv_stats']['incorrect']['count']++;
				}
				else {
					$d['csv_stats']['deleted']['message'] .= "Строка $this->line: <strong>Цена была удалена</strong> продукт SKU: ".$product_details->product_sku."<br />";
					$d['csv_stats']['deleted']['count']++;
				}
			}
		}
			...
			if (!$ps_product_price->add($d)) {
				$d['csv_stats']['skipped']['message'] .= "Строка $this->line: <strong>Цена уже существует</strong> продукт SKU: ".$product_details->product_sku."<br />";
				$d['csv_stats']['skipped']['count']++;
			}
			else {
				$d['csv_stats']['added']['message'] .= "Строка $this->line: <strong>Цена была добавлена</strong> продукт SKU: ".$product_details->product_sku."<br />";
				$d['csv_stats']['added']['count']++;
			}
		}
	}
	...
	function ProductTypeXrefUpload(&$d) {
		...
		if (!$ps_product_product_type_xref->add($d)) {
			$d['csv_stats']['incorrect']['message'] .= "Строка $this->line: <strong>Ссылка уже существует</strong> продукт SKU: ".$product_details->product_sku."<br />";
			$d['csv_stats']['incorrect']['count']++;
		}
		else {
			$d['csv_stats']['added']['message'] .= "Строка $this->line: <strong>Ссылка была добавлена</strong> продукт SKU: ".$product_details->product_sku."<br />";
			$d['csv_stats']['added']['count']++;
		}
	}
	...
	function ProductTypeUpload(&$d) {
		...
		if ($csv_product_type->product_type_id) {
			...
			if ($ps_product_type->update($d)) {
				$d['csv_stats']['updated']['message'] .= 'Line '.$this->line.': <strong>Обновленно</strong> название типа продукта: '.$csv_product_type->product_type_name.'<br />';
				$d['csv_stats']['updated']['count']++;
			}
			else {
				$d['csv_stats']['incorrect']['message'] .= 'Line '.$this->line.': <strong>Обновленно</strong> название типа продукта: '.$csv_product_type->product_type_name.' не мог быть обновлен<br />';
				$d['csv_stats']['incorrect']['count']++;
			}
		}
		else {
			...
			if ($ps_product_type->add($d)) {
				$d['csv_stats']['added']['message'] .= 'Line '.$this->line.': <strong>Добавленно</strong> название типа продукта: '.$csv_product_type->product_type_name.'<br />';
				$d['csv_stats']['added']['count']++;
			}
			else {
				$d['csv_stats']['incorrect']['message'] .= 'Line '.$this->line.': <strong>Неправельное</strong> название типа продукта: '.$csv_product_type->product_type_name.' не был добавлен<br />';
				$d['csv_stats']['incorrect']['count']++;
			}
		}
	}
		...
		if ($csv_product_type_parameters->product_type_id) {
			...
			if ($db->f("total") > 0) {
				if ($ps_product_type_parameters->update_parameter($d)) {
					$d['csv_stats']['updated']['message'] .= 'Line '.$this->line.': <strong>Добавленно</strong> название параметра типа продукта: '.$csv_product_type_parameters->product_type_parameter_name.'<br />';
					$d['csv_stats']['updated']['count']++;
				}
				else {
					$d['csv_stats']['incorrect']['message'] .= 'Line '.$this->line.': <strong>Обновленно</strong> название параметра типа продукта: '.$csv_product_type_parameters->product_type_parameter_name.' could not be updated<br />';
					$d['csv_stats']['incorrect']['count']++;
				}
			}
			else {
				if ($ps_product_type_parameters->add_parameter($d)) {
					$d['csv_stats']['added']['message'] .= 'Line '.$this->line.': <strong>Добавленно</strong> Product type parameter name: '.$csv_product_type_parameters->product_type_parameter_name.'<br />';
					$d['csv_stats']['added']['count']++;
				}
				else {
					$d['csv_stats']['incorrect']['message'] .= 'Line '.$this->line.': <strong>Неправильно</strong> название параметра типа продукта: '.$csv_product_type_parameters->product_type_parameter_name.' не мог быть добавлен<br />';
					$d['csv_stats']['incorrect']['count']++;
				}
			}
		}
		else {
			$d['csv_stats']['incorrect']['message'] .= 'Line '.$this->line.': <strong>Неправильно</strong> название параметра типа продукта: '.$csv_product_type_parameters->product_type_parameter_name.' has no product type linked<br />';
			$d['csv_stats']['incorrect']['count']++;
		}
	}
	...
	function ProductTypeDetailUpload(&$d) {
		...
		if ($product_details->product_id) {
			...
			$q = substr($q, 0, -1);
			$q .= ")";
			if ($this->debug) $this->csv_debug['message'] .= 'Добавление типов деталей нового продукта: <a onclick="switchMenu(\''.$product_details->product_sku.'_add_product_type\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="'.$product_details->product_sku.'_add_product_type" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
			
			if ($db->query($q)) {
				$d['csv_stats']['added']['message'] .= 'Line '.$this->line.': <strong>Добавленно</strong> детали типа продукта SKU: '.$product_details->product_sku.'<br />';
				$d['csv_stats']['added']['count']++;
			}
			else {
				$d['csv_stats']['incorrect']['message'] .= 'Line '.$this->line.': <strong>Неправельно</strong> продукт SKU не мог быть добавлен: '.$product_details->product_sku.'<br />';
				$d['csv_stats']['incorrect']['count']++;
			}
		}
		else {
			$d['csv_stats']['incorrect']['message'] .= 'Line '.$this->line.': <strong>Неправельно</strong> продукт SKU, не найден: '.$product_details->product_sku.'<br />';
			$d['csv_stats']['incorrect']['count']++;
		}
	}
	...
	function EmptyDatabase(&$d) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Пустая таблица продукта: <a onclick="switchMenu(\'product_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_table" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
			$d['csv_stats']['added']['message'] .= 'Таблица продукта была очищена<br />';
			$d['csv_stats']['added']['count']++;
		}
		else {
			$d['csv_stats']['incorrect']['message'] .= 'Таблица продукта не была очищена<br />';
			$d['csv_stats']['incorrect']['count']++;
		}
		$q = "TRUNCATE TABLE `#__{vm}_product_price`;";
		if ($this->debug) $this->csv_debug['message'] .= 'Пустая таблица цены на продукт: <a onclick="switchMenu(\'product_price_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_price_table" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
			$d['csv_stats']['added']['message'] .= 'Таблица цены на продукт была очищена<br />';
			$d['csv_stats']['added']['count']++;
		}
		else {
			$d['csv_stats']['incorrect']['message'] .= 'Таблица цены на продукт не была очищена<br />';
			$d['csv_stats']['incorrect']['count']++;
		}
		$q = "TRUNCATE TABLE `#__{vm}_product_mf_xref`;";
		if ($this->debug) $this->csv_debug['message'] .= 'Пустая таблица ссылается на изготовителя продукта: <a onclick="switchMenu(\'product_mf_xref_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_mf_xreftable" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Empty product attribute table: <a onclick="switchMenu(\'product_attribute_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_attribute_table" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Empty category table: <a onclick="switchMenu(\'category_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="category_table" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Empty category link table: <a onclick="switchMenu(\'category_link_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="category_link_table" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Empty attribute for parent products table: <a onclick="switchMenu(\'product_attribute_sku_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_attribute_sku_table" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Empty product category link table: <a onclick="switchMenu(\'product_category_xref\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_category_xref" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Empty product discount table: <a onclick="switchMenu(\'product_discount_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_discount_table" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Empty product type table: <a onclick="switchMenu(\'product_type_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_type_table" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Empty product type parameter table: <a onclick="switchMenu(\'product_type_parameter_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_type_parameter_table" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Empty product type link table: <a onclick="switchMenu(\'product_product_type_xref_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_product_type_xref_table" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
		...
		if ($this->debug) $this->csv_debug['message'] .= 'Empty manufacturer table: <a onclick="switchMenu(\'manufacturer_table\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="manufacturer_table" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		if ($db->query($q)) {
		...
		while ($db->next_record()) {
			...
			if ($this->debug) $this->csv_debug['message'] .= 'Deleting product type name '.$tablename.' table: <a onclick="switchMenu(\'product_type_name_table_'.$tablename.'\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="product_type_name_table_'.$tablename.'" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q_drop).'</div><br />';
	...
	***************************************************************************/
	...
	/**
	 * (С) Федоринов Григорий
	 *
	 *  Правки Nick  http://forum.virtuemart.ru/index.php?topic=443.0
	 * @param unknown_type $d
	 */
		function export_yml( &$d ) {
		...
		$yml.='<yml_catalog date="'.date('Y-m-d H:i').'">'."\n"; 
 		$yml.="<shop>\n";
 		//название магазина
 		$yml.="<name>$mosConfig_sitename</name>\n";
 		//контора
 		$yml.="<company>$mosConfig_sitename</company>\n";
 		$yml.="<url>$mosConfig_live_site</url>\n";
 		$yml.="<currencies>\n";
		...
		$yml.="</currencies>\n";
 		...
 		$sql="SELECT c.category_id AS id, c.category_name AS name, cref.category_parent_id AS parentid "
 		."\n FROM #__{vm}_category c "
 		."\n LEFT JOIN #__{vm}_category_xref cref ON c.category_id = cref.category_child_id  "
		...
		while( $db->next_record() ) {
		$yml.='<category id="'.$db->f('id').'" parentId="'.$db->f('parentid').'" >'.$db->f('name').'</category>'."\n";	
		}
		$yml.="</categories>\n";
		//товары
		$yml.="<offers>\n";
		
		$sql="SELECT p.product_id AS p_id, pr.product_price AS price, c.category_id AS cat_id,  "
			   ."\n p.product_s_desc AS s_desc, p.product_name AS p_name, p.product_thumb_image as image, p.product_desc "
			   ."\n FROM #__{vm}_product p "
			   ."\n INNER JOIN #__{vm}_product_category_xref cref ON cref.product_id=p.product_id "
			   ."\n INNER JOIN #__{vm}_category c ON cref.category_id=c.category_id "
			   ."\n	LEFT OUTER JOIN #__{vm}_product_price pr ON p.product_id = pr.product_id "
			   ."\n WHERE  p.vendor_id = '1' "
        	   ."\n AND pr.shopper_group_id = '5' "
        	   ."\n AND p. product_publish='Y'";
		$db->query($sql);
//
		$i=1;
		while( $db->next_record() ) {
			$url= "$mosConfig_live_site/index.php?page=shop.product_details&amp;flypage=shop.flypage&amp;product_id=".$db->f('p_id')."&amp;option=com_virtuemart&amp;Itemid=30"; 
		
		$yml.='<offer id="'.$i.'" available="true">'."\n";
		$yml.='<url>'.$url.'</url>'."\n";
		$yml.="<price>".$db->f('price')."</price>\n";
		$yml.="<currencyId>RUR</currencyId>\n";
		$yml.="<categoryId>".$db->f('cat_id')."</categoryId>\n";
		$yml.="<picture>$mosConfig_live_site/components/com_virtuemart/shop_image/product/".$db->f('image') ."</picture>\n";
		$yml.="<name>".$db->f('p_name')."</name>\n";
		$yml.="<description>".$db->f('desc')."</description>\n";
		$yml.="</offer>\n";
		$i++;
		}
		$yml.="</offers>\n";
		...
		//output xml
		$filename = "YML_Export_" .date("j-m-Y_H.i"). ".xml";
	...
	function ProductQuery($query_type) {
		...
		if ($query_type == "update") {
			...
			// If the user is not using the product publish field, we set it to yes
			$q .= "WHERE product_sku='".$product_details->product_sku."'";
			if ($this->debug) $this->csv_debug['message'] .= 'Updating the product with new data: <a onclick="switchMenu(\''.$product_details->product_sku.'_update_product\');" title="Показать/скрыть запрос">Показать скрыть </a><div id="'.$product_details->product_sku.'_update_product" style="display: none; border: 1px solid #000000; padding: 5px;">'.htmlentities($q).'</div><br />';
		}
		else if ($query_type == "add") {
			// If the user is not using the product publish field, we set it to yes
