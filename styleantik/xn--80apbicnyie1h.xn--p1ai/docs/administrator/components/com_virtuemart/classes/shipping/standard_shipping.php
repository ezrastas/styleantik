	...
	**************************************************************************/
	function list_rates( &$d ) {
		...
		if( !empty( $html )) {
			...
			$vmLogger->debug( "Модуль доставки  '".$this->classname."' не может 
				Найти стоимость доставки по следующим данным заказа:
				Вес: ".$d['weight']."
				Страна: $country
				Индекс: $zip" );
		}
	...
	function rate_id_valid($rate_id, $country, $zip, $weight) {
		...
		if ($db->next_record()) {
			...
			if (!stristr($db->f("shipping_rate_country"),$country)
				...
				$vmLogger->debug( 'Страна '.$country.' не поддерживается.');
				$valid = false;
			}
			if($db->f("shipping_rate_weight_start") > $weight) {
				$vmLogger->debug( 'Вес '.$weight.' не поддерживается.');
				$valid = false;
				
			}
			if( $db->f("shipping_rate_weight_end")  < $weight ) {
				$vmLogger->debug( 'Вес '.$weight.' слишком велик для этой несущей.');
				$valid = false;
				
			}
			if (is_numeric($zip)) {
				if( $db->f("shipping_rate_zip_start") > $zip ) {
					$vmLogger->debug( 'Индекс '.$zip.' меньше, чем предельно допустимый для этой несущей.');
					$valid = false;
					
				}
				if( $db->f("shipping_rate_zip_end") < $zip) {
					$vmLogger->debug( 'Индекс '.$zip.' больше чем предельно допустимый для этой несущей.');
					$valid = false;			
				}
			}	
			if( !$valid ) {
