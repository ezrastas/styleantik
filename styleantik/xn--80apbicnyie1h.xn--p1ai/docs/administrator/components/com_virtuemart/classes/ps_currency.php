...
*************************************************************************/
 class ps_currency {
   ...
   function validate_add($d) {
     ...
     if (!$d["currency_name"]) {
       $this->error = "ОШИБКА: Вы должны ввести название для валюты.";
       return False;	
     }
     if (!$d["currency_code"]) {
       $this->error = "ОШИБКА: Вы должны ввести код для валюты.";
       return False;	
     }
     ...
     if ($d["currency_name"]) {
       ...
       $db->next_record();
       if ($db->f("rowcnt") > 0) {
	 $this->error = "Данное название валюты уже существует.";
	 return False;
       }      
     }
     return True;    
  ...
  ***************************************************************************/   
  function validate_delete($d) {
    ...
    if (!$d["currency_id"]) {
      $this->error = "ОШИБКА: Пожалуйста выберите валюту, чтобы удалить.";
      return False;
    }
    else {
      return True;
