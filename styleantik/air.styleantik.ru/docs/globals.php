...
/**
 * Используйте 1 для эмуляции режима register_globals = on
 * ПРЕДУПРЕЖДЕНИЕ: ПАРАМЕТР 1 МОЖЕТ БЫТЬ НЕОБХОДИМ ДЛЯ ОБРАТНОЙ СОВМЕСТИМОСТИ
 * С НЕКОТОРЫМИ КОМПОНЕНТАМИ СТОРОННИХ РАЗРАБОТЧИКОВ, НО ИСПОЛЬЗОВАНИЕ ЭТОГО 
 * ПАРАМЕТРА НЕ РЕКОМЕНДУЕТСЯ
 * 
 * Используйте 0 для эмуляции режима regsiter_globals = off
 * ПРИМЕЧАНИЕ: ЭТО РЕКОМЕНДУЕМЫЙ ПАРАМЕТР ДЛЯ ВАШЕГО САЙТА, НО ОН МОЖЕТ ВЫЗВАТЬ ПРОБЛЕМЫ В РАБОТЕ
 * КОМПОНЕНТОВ СТОРОННИХ РАЗРАБОТЧИКОВ
 */
