...
if (!$is_moderator) { die ("Вы не модератор!! <br/> Эта ошибка зарегистрирована, и ваш адрес IP отослан Суперадминистратору этого сайта; извените..");}
...
switch ($action) {
	case _MOD_DELETE:
		switch (jbDeletePosts($database,$cid)) {
			case -1:
				mosRedirect(JB_LIVEURL.'func=review&amp;catid='.$catid, "ОШИБКА: пост был удален, но текст не мог быть удален проверте #__sb_messages_text в таблицах для mesid IN". explode(',',$cid));
				break;
			case 0:
