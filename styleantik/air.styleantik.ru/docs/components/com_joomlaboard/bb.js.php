<?php
/**
* bb.js.php prints out most javascript used by joomlaboard
* should by <default jb url>&amp;no_html=1&amp;func=sb_bbjs
...
iu_help = "<?php echo _IMAGE_DIMENSIONS.": ".$sbConfig['imageWidth']."x".$sbConfig['imageHeight']." - ".$sbConfig['imageSize']." KB";?>";
fu_help = "<?php echo _FILE_TYPES.": ".$sbConfig['fileTypes']." - ".$sbConfig['fileSize']." KB";?>";
...
$message_emoticons = smile::getEmoticons(0,0);
	foreach ($message_emoticons as $key => $value)
	{
		$key = str_replace(')', '\)', $key);
		$key = str_replace('(', '\(', $key);
		echo "messageString = messageString.replace(/".$key."/g,'<img src=\"".JB_DIRECTURL.$value."\" border=\"0\" alt=\"\" />');";
	}
?>
}
