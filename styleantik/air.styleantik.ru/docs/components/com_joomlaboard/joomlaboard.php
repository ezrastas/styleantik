...
if ($sbConfig['regonly'] && !$my->id)
{
   ...
   echo $sbConfig['offline_message'];
	...
	if ($my->id>0) {
		...
		if ($database->getAffectedRows()==0) {		
			...
			// always create one in joomlaboard
   ...
   // Check read permission, stop if the user doesn't have it
	if($catid > 0 && !in_array($catid,$sbSession->allowed)) {
		...
		$obj_jb_tmpl->addVar('jb-footer','credits','');
		$obj_jb_tmpl->addVar('jb-footer','version_info',"Форум ".$sbConfig['version']);
   ...
   switch ($func) {
      ...
      case 'search':
	     ...
	     if (defined('JPATH_BASE')) {
			...
			$cache->setCaching( 1 );
			$obj_jb_search = $cache->call( array( 'jbSearch', 'jbSearch',$searchword,$sbSession->allowed,$limitstart,$sbConfig['messages_per_page'] ) );
    	}
    	else
			$obj_jb_search=&new jbSearch($database,$searchword,$sbSession->allowed,$limitstart,$sbConfig['messages_per_page']);
      	$obj_jb_search->show();
      break;
      #########################################################################################
		case 'markAllRead':
      		setcookie("sboard_settings[prevvisit]",$previousVisit,time()+31536000,'/');
      		mosRedirect(sefRelToAbs(JB_LIVEURL),_GEN_ALL_MARKED);
      #########################################################################################
      case 'markThisRead':
