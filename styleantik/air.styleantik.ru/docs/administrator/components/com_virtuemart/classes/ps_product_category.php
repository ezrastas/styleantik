...
class ps_product_category extends vmAbstractObject {
	...
	function validate_add(&$d) {
		...
		$valid = true;
		if (!$d["category_name"]) {
			$vmLogger->err( "Вы должны ввести название для категории.");
			$valid = False;
		}
		...
		if (!empty( $d['category_thumb_image_url'] )) {
			...
			if (substr( $d['category_thumb_image_url'], 0, 4) != "http") {
				$vmLogger->err( "URL изображения должен начинаться с http." );
				$valid =  false;
			}
	...
	function validate_delete( $category_id, &$d) {
		...
		if (empty( $category_id )) {
			$vmLogger->err( "Пожалуйста выберите удаляюмую категорию." );
			return False;
		}
		...
		if ($db->next_record()) {
			$vmLogger->err( "Это детская категория - пожалуйста удалите сначала прошлые настройки детской категории.");
			return False;
		}
		...
		if( !stristr( $db->f("category_thumb_image"), "http") ) {
			...
			$d["category_thumb_image_action"] = "delete";
			if (!validate_image($d,"category_thumb_image","category")) {
				$vmLogger->err( "Неудалось удалить изображение категории" );
				return false;
			}
		}
	...
	function add( &$d ) {
		...
		if ($this->validate_add($d)) {
			...
			$vmLogger->info( "Успешно добавленна новая категория: ".$d['category_name'].'.');
			return $category_id;
		}
		else {
			return False;
	...
	function update(&$d) {
		...
		if ($this->validate_update($d)) {
			...
			*/
			...
			$vmLogger->info( "Категория успешно обновленна: ".$d['category_name'].'.' );
	...
	*/
	function delete_record( $record_id, &$d ) {
		...
		$vmLogger->info( "Успешно удаленно ID категории: $record_id." );
		return True;
	}
	/**
			...
			switch( $d["task"] ) {
				...
				foreach( $d['category_id'] as $category_id ) {
					if( !is_numeric( $d['order'][$i] ) ) {
						$d['error'] = "Ошибка: Пожалуйста используйте только цифры чтобы получить список!";
						return false;
					}
					$i++;
				}
