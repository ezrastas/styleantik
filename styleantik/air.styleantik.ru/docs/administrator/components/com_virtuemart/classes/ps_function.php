...
class ps_function {
    ...
    */
		...
		if (!$d["function_name"]) {
			$this->error = "ОШИБКА: Вы должны ввести имя для функции.";
			return False;
		}
		if (!$d["module_id"]) {
			$this->error = "ОШИБКА: ОШИБКА: модуль id должен быть определен.";
			return False;
		}
		if (!$d["function_class"]) {
			$this->error = "ОШИБКА: Вы должны ввести имя для класса.";
			return False;
		}
		if (!$d["function_method"]) {
			$this->error = "ОШИБКА: Вы должны ввести имя для метода.";
			return False;
		}
		if (!$d["function_perms"]) {
			$this->error = "ОШИБКА: Вы должны ввести разрешения для метода.";
			return False;
		}
		if ($d["function_name"]) {
			...
			$db->next_record();
			if ($db->f("rowcnt") > 0) {
				$this->error = "Данное имя функции уже существует.";
				return False;
			}
		}
		return True;
	...
	***************************************************************************/
	function validate_delete($d) {
		...
		if (!$d["function_id"]) {
			$this->error = "ОШИБКА: Пожалуйста выберите функцию, чтобы удалить.";
			return False;
		}
		else {
			return True;
