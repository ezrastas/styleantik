			...
			while( $db->next_record() ) {
				...
				if( $dbs->num_rows() > 0 ) {
					while( $dbs->next_record() ) {
						...
						$script .= "states[".$i++."] = new Array( '".$db->f("country_3_code")."','".$dbs->f("state_2_code")."','"
						//.htmlentities($dbs->f("state_name"), ENT_QUOTES,"windows-1251")."' );\n";
						.$dbs->f("state_name")."' );\n";
					}
				}
				else {
	...
	*/
	function writableIndicator( $folder ) {
		...
		foreach( $folder as $dir ) {
			...
			? '<span style="font-weight:bold;color:green;">Доступен для записи</span>'
			: '<span style="font-weight:bold;color:red;">Недоступен для записи</span>';
			echo '<br/>';
		}
		echo '</div>';
	}
