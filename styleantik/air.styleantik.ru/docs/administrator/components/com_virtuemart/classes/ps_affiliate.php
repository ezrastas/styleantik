...
#    global $ps_user;
    ...
    if (empty($d['user_info_id'])) {
      $d['error'] = "Пожалуйста обеспечьте пользовательскую информацию id!";
      return false;
    }
    ...
    if ($db->next_record()) {
        $d['error'] = "Данный пользовательский id уже - филиал!";
        return false;
    }
    else {
...
#    global $ps_user;
  ...
  function list_order($order_status=A, $secure=0, $date=0) {
   ...
   $start_date = mktime(0,0,0,$month,1,$year);
	...
	if($db->next_record()){
	       ...
	       if (!$i) {
	         echo "<option>---------------------- Заказы не отображены ------------------</option>\n";
	       }
	    	echo "</SELECT>\n";    
  ...
  ***************************************************************************/    
  function get_stats($date=0,$affiliate_id){
    ...
    $stats_string = "Добавление статистики для ".$company_details["company"]." : ".date("F Y")."\n";
	$stats_string .="---------------------------------------------------------------------\n";
	$stats_string .="ID Вашего филиала            =".$company_details["id"]."\n";
	$stats_string .="Ваша текущая норма комиссии = ".$company_details["rate"]."%\n";
	$stats_string .="-----------------------------------------\n";
	$stats_string .="Число направлений    = " .$affiliate_details["visitors"]."\n"; 
	$stats_string .="Число рассматриваемых страниц = " .$affiliate_details["pages"]."\n";
	$stats_string .="Число сделанных заказов  = " .$affiliate_details["orders_made"]."\n"; 
	$stats_string .="Прибыль от всего заказа    = " .$affiliate_details["orders_total"]."\n"; 
	$stats_string .="-----------------------------------------\n";
	$stats_string .="Заработано на комиссии      = " .$affiliate_details["commission_total"]."\n"; 
	$stats_string .="-----------------------------------------\n";
	$stats_string .="Учтите, что комиссия будет выплачиваться\n";
	$stats_string .="только в полностью выполненых заказах.\n";
	$stats_string .="Эти детали должны обеспечить отчетность\n";
	$stats_string .="по заработанной комиссии, когда все заказы\n";
	$stats_string .="будут завершены и отправлены.\n";	
	return $stats_string;
  ...
  ***************************************************************************/    
  ...
  if($i==$j){
   $email_status = "Отправка $i филиала завершена - Продолжение посылки ....";
  }
   ...
   $db = new ps_DB;
	...
	$db->query($q);
        ...
        echo $db->f("first_name") ." ". $db->f("first_name") ." (".$db->f("username").")";
	       ...
	       if (!$i) {
	         echo "<option>---------------------- Нет файлов для отображения ------------------</option>\n";
	       }
	    	echo "</select>\n";    
