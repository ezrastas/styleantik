...
*************************************************************************/
  ...
  ***************************************************************************/  
   function validate_add($d) {
     ...
     if (!$d["country_name"]) {
       $this->error = "ОШИБКА: Вы должны ввести название для страны.";
       return False;	
     }
     if (!$d["country_2_code"]) {
       $this->error = "ОШИБКА: Вы должны ввести код, 2 символа для страны.";
       return False;	
     }
    if (!$d["country_3_code"]) {
      $this->error = "ОШИБКА: Вы должны ввести код, 3 символа для страны.";
      return False;	
    }
     ...
     if ($d["country_name"]) {
       ...
       $db->next_record();
       if ($db->f("rowcnt") > 0) {
	 $this->error = "Данное название страны уже существует.";
	 return False;
       }      
     }
     return True;    
  ...
  ***************************************************************************/   
  function validate_delete($d) {
    ...
    if (!$d["country_id"]) {
      $this->error = "ОШИБКА: Пожалуйста выберите страну, чтобы удалить.";
      return False;
    }
    else {
      return True;
  ...
  ***************************************************************************/   
  ...
  function addState( &$d ) {
    ...
    if ( empty($d['country_id']) ) {
      $d["error"] = "Ошибка: Страна не была выбранна для этого государства";
      return False;
    }
  ...
  function updateState( &$d ) {
    ...
    if (empty($d['state_id']) ||empty($d['country_id']) ) {
      $d["error"] = "Пожалуйста выберите государство или страну для обновления!";
      return False;	
    }
  ...
  function deleteState( &$d ) {
    ...
    if (empty( $d['state_id'])) {
      $d["error"]= "Пожалуйста выберите государство, чтобы удалить!";
      return false;
    }
