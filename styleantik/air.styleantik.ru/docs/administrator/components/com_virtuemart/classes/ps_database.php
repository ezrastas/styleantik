...
class ps_DB extends database {
	...
	*/
	function query( $q='' ) {
		...
		if (empty($q) ) {
			if( empty($this->_sql)) {
				$vmLogger->debug( '"'.__CLASS__.'::'.__FUNCTION__.'" вызван без ждущего обработки вопроса.');
			}
			elseif( !$this->_query_set ) {
				$vmLogger->debug( '"'.__CLASS__.'::'.__FUNCTION__.'": Без запросов управление дважды изменило текст программы SQL.');
			}
		}
		else {
			$this->setQuery( $q );
	...
	function next_record() {
		global $vmLogger;
		if ( empty( $this->_sql ) ) {
			$vmLogger->debug( '"'.__CLASS__.'::'.__FUNCTION__.'()" вызван без задержки вопросом.' );
			return false;
		}
		if ( $this->called ) {
			$this->row++;
