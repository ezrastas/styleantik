...
*
* http://virtuemart.net
* Федоринов Григорий http://virtuemart.ru
*/
...
class flex {
	...
	function list_rates( &$d ) {
		...
		if($order_total < $base_ship) {
			...
			$flat_charge *= $taxrate;
			$shipping_rate_id = urlencode($this->classname."|STD|Стандарная доставка меньше ".$base_ship."|".$flat_charge);
			$html = "";
			...
			$shipping_rate_id = urlencode($this->classname."|STD|Стандартная доставка больше ".$base_ship."|".$shipping_temp1);
			$html = "";
			$html .= "\n<input type=\"radio\" name=\"shipping_rate_id\" id=\"flex_shipping_rate\" checked=\"checked\" value=\"$shipping_rate_id\" />\n";
			$html .= "<label for=\"flex_shipping_rate\">Стандартная доставка: ";
    ...
    */
    ...
    ?>
      <table>
    <tr>
        <td><strong>Минимальная стоимость заказа, когда применяется процентная стоимость доставки:</strong></td>
		<td>
            <input type="text" name="FLEX_BASE_AMOUNT" class="inputbox" value="<?php echo FLEX_BASE_AMOUNT ?>" />
		</td>
		<td>
        <?php echo mm_ToolTip("Минимальная стоимость заказа, когда взымается стоимость доставки в процентах.") ?> 
        </td>
    </tr>
    <tr>
        <td><strong>Минимальная стоимость доставки:</strong>
		</td>
		<td>
            <input type="text" name="FLEX_MIN_CHG" class="inputbox" value="<?php echo FLEX_MIN_CHG ?>" />
		</td>
		<td>
        <?php echo mm_ToolTip("Это минимальная стоимость доставки, применяется если сумма заказа меньше, чем  минимальная стоимость заказа.") ?>
        </td>
    </tr>
    <tr>
        <td><strong>Процентная ставка от стоимости заказа:</strong>
		</td>
		<td>
            <input type="text" name="FLEX_SHIP_PERC" class="inputbox" value="<?php echo FLEX_SHIP_PERC ?>" />
		</td>
		<td>
            <?php echo mm_ToolTip("Это процент от общей суммы заказа, который будет взят в качестве платы за доставку.") ?>
        </td>
    </tr>
	<tr>
        <td><strong>Фиксированная сумма доставки, всегда добавляется к стоимости доставки:</strong>
		</td>
		<td>
            <input type="text" name="FLEX_HAND_FEE" class="inputbox" value="<?php echo FLEX_HAND_FEE ?>" />
		</td>
		<td>
            <?php echo mm_ToolTip("Если вы хотите использовать фиксированную плату, которая будет добавлятся к расчитанной по данным выше сумме доставки, то укажите ее здесь.") ?>
        </td>
    </tr>
	  <tr>
  ...
  */
	...
	*/
	function write_configuration( &$d ) {
		...
		$config = "<?php\n";
		$config .= "defined('_VALID_MOS') or die('Прямой доступ запрещен.'); \n\n";
		...
		if ($fp = fopen(CLASSPATH ."shipping/".$this->classname.".cfg.php", "w")) {
			...
			$vmLogger->err( "Ошибка в записи файла конфигурации" );
			return false;
		}
	}
}
