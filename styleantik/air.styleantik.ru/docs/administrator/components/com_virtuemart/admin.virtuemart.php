...
if (isset($_REQUEST['install_type']) && file_exists( $mosConfig_absolute_path.'/administrator/components/'.$option.'/install.php' )) {
	...
	if( !@unlink( $installfile ) ) {
		echo "<br /><span class=\"message\">Произошла ошибка при удалении файла <strong>install.php</strong>!<br />";
		echo "Вы должны удалить его вручную прежде чем использовать ВМРЕ!</span>";
	}
...
if (defined('_DONT_VIEW_PAGE') && !isset($install_type) ) {
 echo "<script type=\"text/javascript\">alert('$error. Ваши права: ".$_SESSION['auth']['perms']."')</script>\n";
}
...
.' (<a href="http://virtuemart.ru/index2.php?option=com_versions&amp;catid=1&amp;myVersion='.@$VMVERSION->RELEASE.'" onclick="javascript:void window.open(\'http://virtuemart.ru/index2.php?option=com_versions&catid=1&myVersion='.$VMVERSION->RELEASE .'\', \'win2\', \'status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=580,directories=no,location=no\'); return false;" title="Проверить наличие обновлений" target="_blank">Проверить наличие обновлений</a>)</div>';
