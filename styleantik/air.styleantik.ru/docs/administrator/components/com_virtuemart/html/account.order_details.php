...
if ( $db->f("order_status") == "P" ) {
	// Копируем объект базы данных для предотвращения повторяющихся данных
	$db_temp = ps_DB::_clone( $db );
 /** Начинаем вывод  кода HTML формы (Дополнительная информация о платеже/Payment Extra Info) **/ ?>
<table width="100%">
  <tr>
    ...
    echo DEBUG ? vmCommonHTML::getInfoField('Начинаем анализировать дополнительную информацию о платеже...' ) : '';

    // Здесь место где включается эта информация 
    // Спасибо за решение Steve 
    if( eval('?>' . $dbpm->f("payment_extrainfo") . '<?php ') === false ) {
    	echo vmCommonHTML::getErrorField( "Ошибка: Код дополнительной информации платежа ".$dbpm->f( 'payment_method_name').' ('.$dbpm->f('payment_method_code').') '
    	.' содержит ошибки!<br />Пожалуйста  сначало исправьте их' );
    }
      ?>
    </td>
  </tr>
...
/** КОНЕЦ вывода кода HTML формы (Дополнительная информация о платеже/Payment Extra Info) **/
?>
<table border="0" cellspacing="0" cellpadding="2" width="100%">
  <!-- начало информации о пкупателе--> 
  ...
  <!-- КОНЕЦ информации о покупателе--> 
  <!-- НАЧАЛО 2 колонок реквизиты/доставка--> 
  <tr class="sectiontableheader"> 
    <th align="left" colspan="2"><?php echo $VM_LANG->_PHPSHOP_ORDER_PRINT_CUST_INFO_LBL ?></th>
  </tr>
  <tr valign="top"> 
    <td width="50%"> <!-- НАЧАЛО Реквизитов-->
    ...
    <!-- Если вы не хотите показывать ДОПОЛНИТЕЛЬНЫЕ ПОЛЯ (EXTRA FIELD) добавьте в условие "false && ".
         Например: if( false && $VM_LANG->_PHPSHOP_SHOPPER_FORM_EXTRA_FIELD_1 != "" ) -->
    <!-- ДОПОЛНИТЕЛЬНОЕ ПОЛЕ 1 (EXTRA FIELD 1) - НАЧАЛО - Вы можете перенести эту секцию в другую позицию формы. -->
        ...
        <?php } ?>
    <!-- ДОПОЛНИТЕЛЬНОЕ ПОЛЕ 1 (EXTRA FIELD 1) - КОНЕЦ -->
    <!-- ДОПОЛНИТЕЛЬНОЕ ПОЛЕ 2 (EXTRA FIELD 2) - НАЧАЛО - Вы можете перенести эту секцию в другую позицию формы. -->
        ...
        <?php } ?>
    <!-- ДОПОЛНИТЕЛЬНОЕ ПОЛЕ 2 (EXTRA FIELD 2) - КОНЕЦ -->
    <!-- ДОПОЛНИТЕЛЬНОЕ ПОЛЕ 3 (EXTRA FIELD 3) - НАЧАЛО - Вы можете перенести эту секцию в другую позицию формы. -->
