...
?>
	...
	<?php if ($this->cart->getInCheckOut() || $this->address_type == 'ST') {
		$buttonclass = 'default';
	}
	else {
		$buttonclass = 'button vm-button-correct';
	} ?>
		<button class="btn default cancel" type="button" onclick="window.location.href='<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=' . $rview.'&task=cancel'); ?>'"><i class="fa fa-chevron-left"></i> <?php echo vmText::_ ('COM_VIRTUEMART_CANCEL'); ?></button>
		<?php if (VmConfig::get ('oncheckout_show_register', 1) && $this->userDetails->JUser->id == 0 && $this->address_type == 'BT' and $rview == 'cart') { ?>
			<button name="register" class="btn <?php echo $buttonclass ?>" type="button" title="<?php echo vmText::_ ('COM_VIRTUEMART_REGISTER_AND_CHECKOUT'); ?>"><?php echo vmText::_ ('COM_VIRTUEMART_REGISTER_AND_CHECKOUT'); ?></button>
			<?php if (!VmConfig::get ('oncheckout_only_registered', 0)) { ?>
			<button name="save" class="btn <?php echo $buttonclass ?>" title="<?php echo vmText::_ ('COM_VIRTUEMART_CHECKOUT_AS_GUEST'); ?>" type="button"><?php echo vmText::_ ('COM_VIRTUEMART_CHECKOUT_AS_GUEST'); ?></button>
			<?php } ?>
		<?php } else { ?>
			<button name="save_2" class="btn <?php echo $buttonclass ?>" type="button"><?php echo vmText::_ ('COM_VIRTUEMART_SAVE'); ?></button>
		<?php } ?>
	</div>
<script>
